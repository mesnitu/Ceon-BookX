	  ...
	  var isbnOrig = document.forms["new_product"].isbn.value;
		if (isbnOrig != "") {
			...
			if (13 == isbnOrig.length) {
				...
				document.getElementById("isbn_display").innerHTML =  isbnOrig.substring(0,3) + '-' + isbnOrig.substring(3,4) + '-' + isbnOrig.substring(4,10) + '-' + isbnOrig.substring(10,12) + '-' + isbnOrig.substring(12);
			}
		}
	}
            ...
            <td><?php
	            ...
	            switch (true) {
	            	case (empty($pInfo->products_status)):
	            		echo '<span class="alert">' . TEXT_PRODUCT_NOT_DISPLAYED_DUE_TO_PRODUCT_STATUS . '</span>';
	            	break;
	            	...
	            	case (0 == $pInfo->products_quantity && !empty($pInfo->publishing_date) && $pInfo->pub_date_diff >= -intval($bookx_upcoming_products_look_ahead_number_of_days)):
	            		echo '<span class="bookx_article_status_explain">' . sprintf(TEXT_PRODUCT_DISPLAYED_AS_UPCOMING, $bookx_upcoming_products_look_ahead_number_of_days) . '</span>';
	            	break;

	            	case (0 == $pInfo->products_quantity && !empty($pInfo->products_date_available) && $pInfo->products_date_available > date('Y-m-d')):
	            		echo '<span class="bookx_article_status_explain">' . sprintf(TEXT_PRODUCT_DISPLAYED_AS_UPCOMING_WITH_DATE_AVAILABLE) . '</span>';
	            		break;
	            	...
	            	case (0 < $pInfo->products_quantity && !empty($pInfo->publishing_date) && 0 < $pInfo->pub_date_diff && abs($pInfo->pub_date_diff) <= intval($bookx_new_product_look_back_number_of_days)):
	            		echo '<span class="bookx_article_status_explain">' . sprintf(TEXT_PRODUCT_DISPLAYED_AS_NEW, $bookx_new_product_look_back_number_of_days) . '</span>';
	            		break;
            		...
            		case (0 == $pInfo->products_quantity && empty($pInfo->date_available) && (empty($pInfo->publishing_date) || (!empty($pInfo->publishing_date) && 0 < $pInfo->pub_date_diff && abs($pInfo->pub_date_diff) > intval($bookx_new_product_look_back_number_of_days) ))):
            			echo '<span class="bookx_article_status_explain">' . TEXT_PRODUCT_CONSIDERED_OUT_OF_PRINT . '</span>';
            			break;
...
?>
		...
		<!-- *** Field "Subtitle" starts here *** -->
          ...
          <tr class="bookx-data">
            ...
            <td class="main"><?php echo zen_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . zen_draw_input_field('isbn', $pInfo->isbn, 'onchange="checkISBN()" ' . zen_set_field_length(TABLE_PRODUCT_BOOKX_EXTRA, 'isbn'));
